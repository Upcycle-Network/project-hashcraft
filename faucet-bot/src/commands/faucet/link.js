  ...
  execute: async function (interaction) {
    ...
    interaction.client.db.query(`insert ignore into Faucet (userid) values (?); select wallet_name from Faucet where userid = ?; select userid from Faucet where wallet_name = ?`, [u, u, walletname], async function (err, result) {
      ...
      if (!result[1][0].wallet_name) {
        http.get(`http://server.duinocoin.com/v2/users/${interaction.options.get('account-name').value}`, async (res) => {
          ...
          res.on("end", async () => {
            try {
              ...
              errorHandler.APIError(interaction, "Wallet name cannot be verified. Please try again in some time.", "JSON parse fail");
            }
          });
